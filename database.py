...
#2014/01/09 eeelin 新建
#2014/06/05 eeelin 代理接口重构修改
...
try:
    from testbase import version
except ImportError:
    version = "3.0.0"
    
if version >= "5.0.0":
    from testbase.conf import settings
    default_dstloc = settings.QT4S_DEFAULT_LOCATION
else:
    from config import qt4s_config
    default_dstloc = qt4s_config['location']['destination']['default']
...
class SelectStmt(object):
    ...
    def to_sql(self):
        ...
        if self._filters:
            sql += ' WHERE (%s)' %(','.join(self._filters))
        print sql
        return sql
...
class Database(object):
    '''数据库使用接口
    '''
    def __init__(self, host, port, user, passwd, database, location=None ):
        if location is None:
            location = default_dstloc
    ...
    def _get_conn(self):
        if self._conn is None:
            if self._proxy:
                with self._proxy:
                    self._conn = mysql.connect(host=self._host, port=self._port, user=self._user, passwd=self._passwd, db=self._database)
            else:
                self._conn = mysql.connect(host=self._host, port=self._port, user=self._user, passwd=self._passwd, db=self._database)
        return self._conn
